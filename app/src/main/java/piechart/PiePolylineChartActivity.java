...
public class PiePolylineChartActivity extends DemoBase implements OnSeekBarChangeListener,
    ...
    private void setData(int count, float range) {
        ...
        for (int i = 0; i < count; i++) {
            entries.add(new PieEntry((float) (Math.random() * mult) + mult / 5, mNutrients[i % mNutrients.length]));
        }
    ...
    private SpannableString generateCenterSpannableText() {
        ...
        SpannableString s = new SpannableString("Nutrients\non the selected Food");
        s.setSpan(new RelativeSizeSpan(1.7f), 0, 9, 0);
        s.setSpan(new StyleSpan(Typeface.NORMAL), 9, s.length() - 4, 0);
        s.setSpan(new ForegroundColorSpan(Color.GRAY), 9, s.length() - 4, 0);
        s.setSpan(new RelativeSizeSpan(.8f), 9, s.length() - 4, 0);
